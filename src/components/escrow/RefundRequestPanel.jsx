...
export default function RefundRequestPanel({ escrow, currentUser, onUpdate }) {
  ...
  const handleAdminDecision = async (approved, notes = '') => {
    ...
    try {
      const updates = {
        refund_request: { ...escrow.refund_request, admin_approved: approved, admin_notes: notes }
      };
      if (approved) {
      ...
      await base44.entities.Notification.create({
        ...
        message: approved ? 'Your refund request has been approved by admin' : 'Your refund request has been denied',
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1">
            <h3 className="text-lg font-semibold text-red-900">Refund Request Pending Approval</h3>
            <p className="text-sm text-red-700 mt-1">Requested by: {escrow.refund_request.requested_by}</p>
          </div>
        ...
        <div className="flex gap-3">
          <Button onClick={() => handleAdminDecision(false, 'Refund request denied by admin')} disabled={isSubmitting}
          ...
          <Button onClick={() => handleAdminDecision(true, 'Refund approved by admin')} disabled={isSubmitting}
        ...
        <div className="space-y-3">
          <div>
            ...
            <Textarea value={reason} onChange={(e) => setReason(e.target.value)}
              placeholder="Explain why you're requesting a refund..." className="mt-1 text-sm" rows={3} />
          </div>
          <Button onClick={handleRequestRefund} disabled={!reason.trim() || isSubmitting}
            size="sm" className="w-full bg-red-500 hover:bg-red-600">
            {isSubmitting ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <AlertCircle className="w-4 h-4 mr-2" />}
