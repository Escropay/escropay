      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div className="flex-1">
          ...
          {escrow.modification_request?.reason && (
            <p className="text-sm text-orange-700 mt-1"><strong>Reason:</strong> {escrow.modification_request.reason}</p>
          )}
          {escrow.modification_request?.requested_changes && (
            <p className="text-sm text-orange-700 mt-1"><strong>Requested changes:</strong> {escrow.modification_request.requested_changes}</p>
          )}
        </div>
      </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="sm:col-span-2 space-y-1">
              ...
              <Input value={formData.title} onChange={e => setFormData(p => ({ ...p, title: e.target.value }))} placeholder="Title" />
            </div>
            <div className="space-y-1">
              <Label>Amount ({currency.code})</Label>
              <Input type="number" step="0.01" min="0" value={formData.amount}
                onChange={e => setFormData(p => ({ ...p, amount: e.target.value }))} placeholder="0.00" />
            </div>
            <div className="space-y-1">
            ...
            <div className="sm:col-span-2 space-y-1">
              ...
              <Textarea value={formData.description} onChange={e => setFormData(p => ({ ...p, description: e.target.value }))}
                placeholder="Describe the updated terms..." rows={3} />
            </div>
          </div>
          <div className="flex gap-3">
            <Button variant="outline" onClick={() => setIsEditing(false)}>Cancel</Button>
            <Button onClick={handleSubmit} disabled={isSubmitting || !formData.title || !formData.amount}
